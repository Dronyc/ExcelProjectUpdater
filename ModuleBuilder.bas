...
Public Sub SetAllProjectFormulas(ByVal lo As ListObject)
    LogStep "Установка формул: Проверка Контрагент"
    SetColumnFormula lo, "Проверка Контрагент", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);ЛОЖЬ;ИЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Контрагент]]))=0;ДЛСТР(СЖПРОБЕЛЫ([@[Контрагент]]))=1;И(ДЛСТР(СЖПРОБЕЛЫ([@[Контрагент]]))>1;СОВПАД(СЖПРОБЕЛЫ([@[Контрагент]]);ПОВТОР(ЛЕВСИМВ(СЖПРОБЕЛЫ([@[Контрагент]]);1);ДЛСТР(СЖПРОБЕЛЫ([@[Контрагент]])))))))"
    
    LogStep "Установка формул: Проверка Дата создания"
    SetColumnFormula lo, "Проверка Дата создания", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);ЛОЖЬ;ИЛИ(ЕПУСТО([@[Дата создания]]);НЕ(ЕЧИСЛО([@[Дата создания]]))))"
    
    LogStep "Установка формул: Проверка Код"
    SetColumnFormula lo, "Проверка Код", "=ЕСЛИ(И(ЕПУСТО([@[Дата создания]]);СЖПРОБЕЛЫ([@[Код в базе 1С:УПП]])="""");ЛОЖЬ;ИЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Код в базе 1С:УПП]]))=0;ЛЕВСИМВ(СЖПРОБЕЛЫ([@[Код в базе 1С:УПП]]);2)<>""Я-""))"
    
    LogStep "Установка формул: Проверка Код проекта в 1С:УПП"
    SetColumnFormula lo, "Проверка Код проекта в 1С:УПП", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);ЛОЖЬ;ИЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Код проекта в 1С:УПП]]))=0;ДЛСТР(СЖПРОБЕЛЫ([@[Код проекта в 1С:УПП]]))=1;И(ДЛСТР(СЖПРОБЕЛЫ([@[Код проекта в 1С:УПП]]))>1;СОВПАД(СЖПРОБЕЛЫ([@[Код проекта в 1С:УПП]]);ПОВТОР(ЛЕВСИМВ(СЖПРОБЕЛЫ([@[Код проекта в 1С:УПП]]);1);ДЛСТР(СЖПРОБЕЛЫ([@[Код проекта в 1С:УПП]])))))))"
    
    LogStep "Установка формул: Проверка Наименование проекта"
    SetColumnFormula lo, "Проверка Наименование проекта", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);ЛОЖЬ;ИЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Наименование проекта]]))=0;ДЛСТР(СЖПРОБЕЛЫ([@[Наименование проекта]]))=1;И(ДЛСТР(СЖПРОБЕЛЫ([@[Наименование проекта]]))>1;СОВПАД(СЖПРОБЕЛЫ([@[Наименование проекта]]);ПОВТОР(ЛЕВСИМВ(СЖПРОБЕЛЫ([@[Наименование проекта]]);1);ДЛСТР(СЖПРОБЕЛЫ([@[Наименование проекта]])))))))"
    
    LogStep "Установка формул: Проверка Состояние"
    SetColumnFormula lo, "Проверка Состояние", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);ЛОЖЬ;ИЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Состояние]]))=0;ДЛСТР(СЖПРОБЕЛЫ([@[Состояние]]))=1;И(ДЛСТР(СЖПРОБЕЛЫ([@[Состояние]]))>1;СОВПАД(СЖПРОБЕЛЫ([@[Состояние]]);ПОВТОР(ЛЕВСИМВ(СЖПРОБЕЛЫ([@[Состояние]]);1);ДЛСТР(СЖПРОБЕЛЫ([@[Состояние]])))))))"
    
    LogStep "Установка формул: Недопустимое Состояние"
    SetColumnFormula lo, "Недопустимое Состояние", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);ЛОЖЬ;ЕСЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Состояние]]))=0;ЛОЖЬ;ЕОШИБКА(ПОИСКПОЗ(СЖПРОБЕЛЫ([@[Состояние]]);ДопустимыеСостояния;0))))"
    
    LogStep "Установка формул: Проверка Куратор"
    SetColumnFormula lo, "Проверка Куратор", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);ЛОЖЬ;ИЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Куратор]]))=0;ДЛСТР(СЖПРОБЕЛЫ([@[Куратор]]))=1;И(ДЛСТР(СЖПРОБЕЛЫ([@[Куратор]]))>1;СОВПАД(СЖПРОБЕЛЫ([@[Куратор]]);ПОВТОР(ЛЕВСИМВ(СЖПРОБЕЛЫ([@[Куратор]]);1);ДЛСТР(СЖПРОБЕЛЫ([@[Куратор]])))))))"
    
    LogStep "Установка формул: Проверка Менеджер ОП"
    SetColumnFormula lo, "Проверка Менеджер ОП", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);ЛОЖЬ;ИЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Менеджер ОП]]))=0;ДЛСТР(СЖПРОБЕЛЫ([@[Менеджер ОП]]))=1;И(ДЛСТР(СЖПРОБЕЛЫ([@[Менеджер ОП]]))>1;СОВПАД(СЖПРОБЕЛЫ([@[Менеджер ОП]]);ПОВТОР(ЛЕВСИМВ(СЖПРОБЕЛЫ([@[Менеджер ОП]]);1);ДЛСТР(СЖПРОБЕЛЫ([@[Менеджер ОП]])))))))"
    
    LogStep "Установка формул: Список некорректных полей"
    SetColumnFormula lo, "Список некорректных полей", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);"""";ОБЪЕДИНИТЬ(""""&"" "";""";ИСТИНА;ЕСЛИ([@[Проверка Контрагент]];""Контрагент"";"""");ЕСЛИ([@[Проверка Дата создания]];""Дата создания"";"""");ЕСЛИ([@[Проверка Код]];""Код в базе 1С:УПП"";"""");ЕСЛИ([@[Проверка Код проекта в 1С:УПП]];""Код проекта в 1С:УПП"";"""");ЕСЛИ([@[Проверка Наименование проекта]];""Наименование проекта"";"""");ЕСЛИ(ИЛИ([@[Проверка Состояние]];[@[Недопустимое Состояние]]);""Состояние"";"""");ЕСЛИ([@[Проверка Куратор]];""Куратор"";"""");ЕСЛИ([@[Проверка Менеджер ОП]];""Менеджер ОП"";"""")))""
    
    LogStep "Установка формул: Ошибка обязательных полей"
    SetColumnFormula lo, "Ошибка обязательных полей", "=ЕСЛИ([@[Список некорректных полей]]="""";ЛОЖЬ;ИСТИНА)"
    
    LogStep "Установка формул: Число ошибок"
    ...
    LogStep "Установка формул: Проект закрыт"
    SetColumnFormula lo, "Проект закрыт", "=ЕСЛИ([@[Дата создания]]="""";"""";ИЛИ([@[Состояние]]=""Завершен"";[@[Состояние]]=""Прекращен с отрицательным результатом"";[@[Состояние]]=""Не состоялся""))"
    
    LogStep "Установка формул: Группа ПГС"
    SetColumnFormula lo, "Группа ПГС", "=ЕСЛИ(ЕПУСТО([@[Продукт]]);"""";ЕСЛИ(ЕЧИСЛО(ПОИСК(""ПГС"";ЕСЛИОШИБКА(ВПР(СЖПРОБЕЛЫ([@[Продукт]]);тблСправочникПродуктов;2;ЛОЖЬ);"""")));""да"";ЕСЛИ(ЕЧИСЛО(ПОИСК(""ТИМ"";ЕСЛИОШИБКА(ВПР(СЖПРОБЕЛЫ([@[Продукт]]);тблСправочникПродуктов;2;ЛОЖЬ);"""")));""да"";"""")))"
    
    LogStep "Установка формул: Группа PLM"
    SetColumnFormula lo, "Группа PLM", "=ЕСЛИ(ЕПУСТО([@[Продукт]]);"""";ЕСЛИ(ЕЧИСЛО(ПОИСК(""PLM"";ЕСЛИОШИБКА(ВПР(СЖПРОБЕЛЫ([@[Продукт]]);тблСправочникПродуктов;2;ЛОЖЬ);"""")));""да"";""""))"
    
    LogStep "Установка формул: Дубль кода"
    SetColumnFormula lo, "Дубль кода", "=ЕСЛИ(ИЛИ(ЕПУСТО([@[Дата создания]]);ДЛСТР(СЖПРОБЕЛЫ([@[Код в базе 1С:УПП]]))=0);ЛОЖЬ;СЧЁТЕСЛИМН([Код в базе 1С:УПП];[@[Код в базе 1С:УПП]];[Статус проверки ручной];""<>Архивировать"")>1)"
    
    LogStep "Установка формул: Дубль ссылки"
    SetColumnFormula lo, "Дубль ссылки", "=ЕСЛИ(ИЛИ(ЕПУСТО([@[Дата создания]]);ДЛСТР(СЖПРОБЕЛЫ([@[Ссылка на проект]]))=0);ЛОЖЬ;СЧЁТЕСЛИМН([Ссылка на проект];[@[Ссылка на проект]];[Статус проверки ручной];""<>Архивировать"")>1)"
    
    LogStep "Установка формул: Статус проверки авто"
    SetColumnFormula lo, "Статус проверки авто", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);"""";ЕСЛИ(ИЛИ([@[Ошибка обязательных полей]];[@[Дубль кода]];[@[Дубль ссылки]]);ЕСЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Ответственный за статус]]))=0;""Не назначен Ответственный за статус"";""Не соответствует требованиям"");ЕСЛИ(ДЛСТР(СЖПРОБЕЛЫ([@[Ссылка на проект]]))=0;""Требуется ссылка"";ЕСЛИ(ИЛИ(СЖПРОБЕЛЫ([@[Состояние]])=""Завершен"";СЖПРОБЕЛЫ([@[Состояние]])=""Прекращен с отрицательным результатом"";СЖПРОБЕЛЫ([@[Состояние]])=""Не состоялся"");""Готов к передаче в архив"";""Соответствует требованиям""))))"
    
    LogStep "Установка формул: Комментарий к статусу авто"
    SetColumnFormula lo, "Комментарий к статусу авто", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);"""";СЖПРОБЕЛЫ(ЕСЛИ([@[Ошибка обязательных полей]];""Некорректные данные в поле: ""&[@[Список некорректных полей]];"""")&ЕСЛИ([@[Дубль кода]];ЕСЛИ([@[Ошибка обязательных полей]];""; "";"""")&""Дублируется Код в базе 1С:УПП"";"""")&ЕСЛИ([@[Дубль ссылки]];ЕСЛИ(ИЛИ([@[Ошибка обязательных полей]];[@[Дубль кода]]);""; "";"""")&""Дублируется Ссылка на проект"";""""))"
    
    LogStep "Установка формул: Статус проверки"
    SetColumnFormula lo, "Статус проверки", "=ЕСЛИ(ЕПУСТО([@[Дата создания]]);"""";ЕСЛИ(ИЛИ([@[Статус проверки ручной]]=""Архивировать"";[@[Статус проверки ручной]]=""Уточнить!"";[@[Статус проверки ручной]]=""В работе"");[@[Статус проверки ручной]];[@[Статус проверки авто]]))"
    
    LogStep "Установка формул: Комментарий к статусу"
    SetColumnFormula lo, "Комментарий к статусу", "=СЖПРОБЕЛЫ([@[Комментарий к статусу авто]]&ЕСЛИ(И([@[Комментарий к статусу авто]]<>""";СЖПРОБЕЛЫ([@[Комментарий к статусу ручной]])<>"""");""; "";"""")&СЖПРОБЕЛЫ([@[Комментарий к статусу ручной]]))"
    
    LogStep "Установка формул: Есть в эталоне"
...
ws.Cells(r, 4).formulaLocal = "=C" & r & "-E" & r
    ...
    ws.Range("E" & (rowT6 + 7)).formulaLocal = "=СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Проверка Состояние],ИСТИНА)+СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Недопустимое Состояние],ИСТИНА)"
...
ws.Cells(r, 4).formulaLocal = "=C" & r & "-E" & r
    ...
    ws.Range("E" & (rowT6b + 7)).formulaLocal = "=СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Проверка Состояние],ИСТИНА,тблПроекты[Группа ПГС],""да"",тблПроекты[Группа PLM],"""")+СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Недопустимое Состояние],ИСТИНА,тблПроекты[Группа ПГС],""да"",тблПроекты[Группа PLM],"""")"
...
ws.Cells(r, 4).formulaLocal = "=C" & r & "-E" & r
    ...
    ws.Range("E" & (rowT6c + 7)).formulaLocal = "=СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Проверка Состояние],ИСТИНА,тблПроекты[Группа PLM],""да"",тблПроекты[Группа ПГС],"""")+СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Недопустимое Состояние],ИСТИНА,тблПроекты[Группа PLM],""да"",тблПроекты[Группа ПГС],"""")"
...
ws.Cells(r, 4).formulaLocal = "=C" & r & "-E" & r
    ...
    ws.Range("E" & (rowT6d + 7)).formulaLocal = "=СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Проверка Состояние],ИСТИНА,тблПроекты[Группа ПГС],""да"",тблПроекты[Группа PLM],""да"")+СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Недопустимое Состояние],ИСТИНА,тблПроекты[Группа ПГС],""да"",тблПроекты[Группа PLM],""да"")"
...
ws.Cells(r, 4).formulaLocal = "=C" & r & "-E" & r
    ...
    ws.Range("E" & (rowT6e + 7)).formulaLocal = "=СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Проверка Состояние],ИСТИНА,тблПроекты[Группа ПГС],"""",тблПроекты[Группа PLM],"""")+СЧЁТЕСЛИМН(тблПроекты[Дата создания],""<>"",тблПроекты[Недопустимое Состояние],ИСТИНА,тблПроекты[Группа ПГС],"""",тблПроекты[Группа PLM],"""")"
